...
int main(void){
            ...
            for (int k = 0; k < dim3; ++k) {
                for (int l = 0; l < dim4; ++l) {
                 ...
                 int tmp= array4d[i][j][k][l];
                    product_test=std::min(product_test,tmp);
    ...
    // Print sum
     std::cout << "min: " << product_test <<std::endl;
   ...
   //OpenCL part
    ...
    std::ifstream sourceFile("min_max.cl");
   ...
   std::vector<int> final_out(TOTAL_SIZE/GROUP_SIZE);
    ...
    cl::Kernel kernel(program, "min_max");
...
cl::NDRange local_size(GROUP_SIZE);
   ...
   // printf("%d ",final_out[i]);
    tmp_sum= std:: min(tmp_sum,final_out[i]);
   }
   printf("min: %d",tmp_sum);
