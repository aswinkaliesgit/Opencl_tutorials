        ...
        for (int j = 0; j < output_width; j++)
        {
            ...
            for (int k = 0; k < kernel_size; k++)
            {
                for (int l = 0; l < kernel_size; l++)
                {
                    ...
                    local_max = std::max(local_max, input_tensor_flattened[input_index]);
                }
            }
            int output_index = i * output_width + j;
            output_tensor_flattened[output_index] = local_max;
        }
    }
 std::string sourceCode;
    std::ifstream sourceFile("Maxpooling.cl");
    ...
    cl::Kernel kernel(program, "Maxpooling");
